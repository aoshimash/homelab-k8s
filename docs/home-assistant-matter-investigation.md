...
K8sノードからSwitchBot Hub 2のmDNSパケットが受信できない問題を調査した結果、**TP-LinkルーターのIGMPスヌーピング**が原因であることが判明。

```bash
# IGMPスヌーピング有効時：SwitchBot Hub 2からのパケットが見えない
$ kubectl exec debug-net -- tcpdump -i enp1s0f1 'host 192.168.0.206'
0 packets captured

# IGMPスヌーピング無効後：パケットが見えるようになった
$ kubectl exec debug-net -- tcpdump -i enp1s0f1 'host 192.168.0.206'
192.168.0.206.5353 > 224.0.0.251.5353: PTR _matterc._udp.local. ...
```

#### IGMPスヌーピングとは

- ネットワークスイッチがマルチキャストトラフィックを効率的に配信する機能
- 有効時：IGMPメンバーシップレポートを監視し、登録済みポートのみにマルチキャストを転送
- 問題点：K8sノードがmDNSグループ（224.0.0.251）に正しく参加登録できていなかった

#### 解決策

TP-Linkルーター管理画面でIGMPスヌーピングを無効化。

### IGMPスヌーピング無効化後の状況

IGMPスヌーピングを無効化した後、K8sノードでSwitchBot Hub 2からのmDNSパケットを正常に受信できるようになった：

```bash
$ kubectl exec debug-net -- tcpdump -i enp1s0f1 'host 192.168.0.206'
192.168.0.206.5353 > 224.0.0.251.5353: PTR _matterc._udp.local., PTR 3523549EA78ECCC4._matterc._udp.local., SRV 64E833EE8F98.local.:5540 0 0, TXT "PI=" "PH=33" ..., A 192.168.0.206, AAAA fe80::66e8:33ff:feee:8f98
```

SwitchBot Hub 2の情報：
- **IPアドレス**: 192.168.0.206
- **Matterポート**: 5540
- **IPv6 Link-Local**: fe80::66e8:33ff:feee:8f98
- **mDNSサービス名**: 3523549EA78ECCC4._matterc._udp.local.

### Matter Server API経由でのペアリング再試行

ネットワークレベルでは通信可能になったため、Matter Server APIを直接呼び出してペアリングを試行：

```python
# WebSocket API経由でコミッショニング
msg = {
...
```
Starting Matter commissioning with code using Node ID 1.
Discovery timed out
Secure Pairing Failed
CHIP Error 0x00000003: Incorrect state
Commission with code failed for node 1.
...
### CHIP SDK minimal_mdns の問題

ネットワークレベルでmDNSパケットが到達しているにもかかわらず、Matter Serverのログには依然として以下のエラーが出力される：
...
1. **CHIP SDKの`minimal_mdns`実装**は、複数のネットワークインターフェースがある環境で問題を起こす
2. **Ciliumの仮想インターフェース**（lxc*, cilium_*）がCHIP SDKのエンドポイント選択を混乱させる
3. `--primary-interface enp1s0f1`オプションを指定しても、SDKは全インターフェースをスキャンして問題を起こす
4. **ネットワーク到達性とSDKの動作は別問題** - パケットが届いていてもSDKが正しく処理できない

### 最終結論

**IGMPスヌーピングの問題は解決したが、CHIP SDK自体がKubernetes + hostNetwork環境で正常に動作しない**

Matter SDKは以下の前提で設計されている：
- シンプルなネットワーク構成（1-2個のインターフェース）
- システムレベルのAvahi/mDNSサポート
- 仮想ネットワークインターフェースがない環境

Kubernetes + Cilium環境では、これらの前提が満たされないため、Matter統合は技術的に困難。
...
| 2026-01-05 | IGMPスヌーピング問題発見・解決、CHIP SDK問題特定 |
